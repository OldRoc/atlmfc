...
void CMDIChildWndEx::RegisterTaskbarTab(CMDIChildWndEx* pWndBefore)
{
	...
	{
		TRACE1("Creation of tab proxy window failed, error code: %d\n", GetLastError());
		return;
	}
	...
	if (pTaskbarList3)
	{
		...
		if (FAILED(hr))
		{
			TRACE1("Registration of tab failed, error code: %x\n", hr);
		...
		if (FAILED(hr))
		{
			TRACE1("Setting of tab order failed, error code: %x\n", hr);
			UnregisterTaskbarTab();
			return;
		}
...
BOOL CMDIChildWndEx::SetTaskbarThumbnailClipRect(CRect rect)
{
	...
	if (!SUCCEEDED(hr))
	{
		TRACE1("SetTaskbarThumbnailClipRect failed with code %x.\n", hr);
		return FALSE;
	}
