...
BOOL CJumpList::InitializeList()
{
	...
	if (m_destListPtr == NULL)
	{
		...
		if (FAILED(hr))
		{
			TRACE1("Error initializing Jump List, code: %x\n", hr);
			return FALSE;
		}
	}
	...
	if (m_tasksPtr == NULL)
	{
		...
		if (FAILED(hr))
		{
			TRACE1("Error initializing task collection, code: %x\n", hr);
			return FALSE;
		}
	}
	...
	if (!m_bInitialized)
	{
		...
		if (FAILED(hr))
		{
			TRACE1("ICustomDestinationList::BeginList failed, initialization canceled, HRESULT: %x\n", hr);
			return FALSE;
		}
		m_bInitialized = TRUE;
	}
...
BOOL CAppDestinations::RemoveDestination(IUnknown* pUnk)
{
	...
	if (FAILED(hr))
	{
		TRACE1("RemoveDestination failed with code %x.\n", hr);
		return FALSE;
	}
...
BOOL CAppDestinations::RemoveAllDestinations()
{
	...
	if (FAILED(hr))
	{
		TRACE1("RemoveAllDestinations failed with code %x.\n", hr);
		return FALSE;
	}
